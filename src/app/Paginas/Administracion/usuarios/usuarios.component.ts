...
export class UsuariosComponent implements OnInit {
  ...
  UsuarioForms = new FormGroup({
    ...
    usr_correo: new FormControl('', Validators.required),
    usr_telefono: new FormControl('', Validators.required),
    ...
    if (lblFiltro.textContent === 'Descripcion') {
      ...
      if (num === 0) {
        const resultado = this.ListaUsuarios.filter((elemento) => {
          return elemento.usr_usuario.includes(nombre.toUpperCase());
        });
