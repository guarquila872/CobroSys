...
import { RecargoI,Tipo_RecargoI } from 'src/app/Modelos/response.interface';
...
GuardarObjeto(datos:any) { 
  ...
  if(datos.rec_id_cartera===2||datos.rec_id_cartera===10||datos.rec_id_cartera===11)
    {
      datos.rec_esactivo =
    ...
    {
      this.alerta.ErrorEnLaPeticion('Con esta cartera no se puede ingresar');
    }
    ...
    this.api
      ...
      .pipe(
        map((tracks) => {
          ...
          console.log(this.ListaTipoRecargo );
        }),
        catchError((error) => {
          this.loading = false;
          this.alerta.ErrorAlRecuperarElementos();
